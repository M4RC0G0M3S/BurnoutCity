...
namespace BurnoutCity.States
{
    public class RaceState : BaseState
    {
        ...
        private Texture2D  _carSprite   = null!;   // sprite do jogador
 
        // ── Sprite do rival (NOVO) ─────────────────────────────────────
        // Caminho: Content/Sprites/CarSprites/<_rivalSpriteName>.png
        ...
        // ═════════════════════════════════════════════════════════════
        //  CONSTRUTOR
        //  Adicionado parâmetro rivalSpriteName para receber o nome
        //  do ficheiro PNG do carro do rival (sem extensão).
        // ═════════════════════════════════════════════════════════════
        public RaceState(PlayerData? playerData  = null,
                         ...
                         string rivalSpriteName  = "rival_default")   // <-- NOVO
        {
            ...
            _rivalSpriteName = rivalSpriteName;   // <-- NOVO
        }
        ...
        public override void LoadContent()
        {
            ...
            // ── Sprite do carro do RIVAL (NOVO) ───────────────────────
            // Tenta carregar o PNG específico do rival.
            // Se não existir, _rivalSprite fica null e usa o fallback retângulo.
            try
            {
            ...
            {
                _rivalSprite = null;
                Console.WriteLine($"[Race] AVISO: sprite rival não encontrado: {_rivalSpriteName} — fallback ativo");
            }
 
            // Câmara começa no início da pista
            _camPos.X = 640f;
        ...
        private void DrawCar(SpriteBatch sb, int cx, int cy, Color color, bool isRival)
        {
            ...
            else if (_rivalSprite != null && isRival)
            {
                ...
                //força o sprite a ocupar CarW x CarH independentemente do tamanho do PNG:
                float scaleX = CarW / (float)_rivalSprite.Width;
                float scaleY = CarH / (float)_rivalSprite.Height;
                // Usa a maior das duas escalas para não ficar esmagado
                float uniformScale = Math.Max(scaleX, scaleY) * 1.5f;

                sb.Draw(
                    texture:         _rivalSprite,
                    position:        new Vector2(cx, cy),
                    sourceRectangle: new Rectangle(0, 0, _rivalSprite.Width, _rivalSprite.Height),
                    color:           Color.White,
            ...
            // ── Fallback retângulo (sem sprite disponível) ─────────────
            else
            {
        ...
        // ── Rival Card (preview antes da corrida) ─────────────────────
        private void DrawRivalCard(SpriteBatch sb, int W, int H)
        {
            ...
            // Cor do rival (ou miniatura do sprite, se disponível)
            if (_rivalSprite != null)
            {
                // Mostra o sprite do rival em miniatura no canto do card
                int thumbSize = 42;
                ...
                sb.Draw(
                    texture:         _rivalSprite,
                    position:        new Vector2(px + pW - 42, py + 11),
                    sourceRectangle: new Rectangle(0, 0, _rivalSprite.Width, _rivalSprite.Height),
                    color:           Color.White,
                    rotation:        MathHelper.PiOver2,
                    origin:          thumbOrigin,
